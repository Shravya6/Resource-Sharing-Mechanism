...
<body id="page-top">
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <li>
                        <a class="page-scroll" href="t2log.jsp">Back</a>
                    </li>
                </ul>
            </div>
            <!-- /.navbar-collapse -->
    ...
    <header>
       <div class="header-content">
			...
			<form action="t2reg1.jsp" method="post"><center>
                <h1 id="homeHeading">Tenant2 Register</h1>
              
                <table>
                    
                    <tr><td><label style="margin-right: 10px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>Tenant 2 ID:</h3></label></td>
				<td><input type="text" style="margin-right: 10px;margin-top: 15px;margin-left: 10px;margin-bottom: 10px;color: #31708f;" required name="t2id" <%
                                    Connection con = DbConnection.getConnection();
    
                Statement st=con.createStatement();
                ResultSet rs=st.executeQuery("select count(id) from t2");
                rs.next();
                int n=rs.getInt(1);
                if(n==0)
                {
                 int s=1; %>
           value="T2-<%=s%>" readonly="readonly"<%}
             else{
                 Statement st1=con.createStatement();
                 ResultSet rs1=st1.executeQuery("select max(cast(substr(id,4,length(id)-3) as unsigned)) from t2");
                 rs1.next();
                 int m=rs1.getInt(1);
                 System.out.println(m);
                 int count=m+1;
           System.out.println(count);%>  
                               value="T2-<%=count%>" title="enter tenant 2 id" readonly="readonly"/><%}%></td></tr>
				...
				<td><input type="radio" name="gender" value="Male" style="margin-right: 10px;margin-top: 15px;margin-left: 10px;margin-bottom: 10px;">Male
				<input type="radio" name="gender" value="Female" style="margin-right: 10px;margin-top: 15px;margin-left: 10px;margin-bottom: 10px;">Female</td></tr></div>
				<div><tr><td><label style="margin-right: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>File Access Policy:</h3></label></td>
				<td><select required name="policy" style="margin-right: 10px;margin-top: 15px;margin-left: 10px;margin-bottom: 10px;color: #31708f;">
				<option value>Choose Policy</option>
				<option value="CSE">CSE</option>
				<option value="ECE">ECE</option>
				<option value="EEE">EEE</option>
				<option value="IT">IT</option></select></td></tr></div>
