...
session.setAttribute("t2tenants", t2tenant);
session.setAttribute("fids", fid);
session.setAttribute("oids", oid);
session.setAttribute("uids", uid);
session.setAttribute("filenames", filename);
session.setAttribute("t1tenants", t1tenant);
%>

			<form action="t2authen2.jsp" method="post"><center>
                <h1 id="homeHeading">AUTHENTICATE THE IDENTITY</h1>
                <hr>
                <table>
				<div><tr><td><label style="margin-right: 10px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>ENTER PRIVATE KEY:</h3></label></td>
				<td><input type="password" required name="pkey" placeholder="enter private key" style="margin-right: 10px;margin-top: 25px;margin-left: 10px;margin-bottom: 10px;color: #31708f;"></td></tr></div>
				</table>
