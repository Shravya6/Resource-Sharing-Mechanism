...
<body id="page-top">
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <li>
                        <a class="page-scroll" href="dataprolog.jsp">Back</a>
                    </li>
                </ul>
            </div>
            <!-- /.navbar-collapse -->
    ...
    <header>
       <div class="header-content">
			...
			<form action="dataproreg1.jsp" method="post"><center>
                <h1 id="homeHeading">DATA PROVIDER REGISTRATION</h1>
              
                <table>
                    
                    <div><tr><td><label style="margin-right: 10px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>Data Provider ID:</h3></label></td>
				<td><input type="text" required name="dpid" style="margin-right: 10px;margin-top: 15px;margin-left: 10px;margin-bottom: 10px;color: #31708f;" <%
                                    Connection con = DbConnection.getConnection();
    
                Statement st=con.createStatement();
                ResultSet rs=st.executeQuery("select count(id) from provider");
                rs.next();
                int n=rs.getInt(1);
                if(n==0)
                {
                 int s=1; %>
           value="DP-<%=s%>" readonly="readonly"<%}
             else{
                 Statement st1=con.createStatement();
                 ResultSet rs1=st1.executeQuery("select max(cast(substr(id,4,length(id)-3) as unsigned)) from provider");
                 rs1.next();
                 int m=rs1.getInt(1);
                 int count=m+1;%>  
                               value="DP-<%=count%>" title="enter data provider id" readonly="readonly"/><%}%> 
                                    </td></tr></div>
