...
<body id="page-top">
    ...
    <header>
       <div class="header-content">
			...
			<form action="t1reg1.jsp" method="post"><center>
                <h1 id="homeHeading">Tenant1 Register</h1>
              
                <table>
				
                    <div><tr><td><label style="margin-right: 10px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>Tenant 1 ID:</h3></label></td>
				<td><input type="text" required name="t1id" style="margin-right: 10px;margin-top: 15px;margin-left: 10px;margin-bottom: 10px;color: #31708f;" <%
                                    Connection con = DbConnection.getConnection();
    
                Statement st=con.createStatement();
                ResultSet rs=st.executeQuery("select count(id) from t1");
                rs.next();
                int n=rs.getInt(1);
                if(n==0)
                {
                 int s=1; %>
           value="T1-<%=s%>" readonly="readonly"<%}
             else{
                 Statement st1=con.createStatement();
                 ResultSet rs1=st1.executeQuery("select max(cast(substr(id,4,length(id)-3) as unsigned)) from t1");
                 rs1.next();
                 int m=rs1.getInt(1);
                 int count=m+1;%>  
                               value="T1-<%=count%>" title="enter tenant 1 id" readonly="readonly"/><%}%></td></tr></div>
                    
                    <div><tr><td><label style="margin-right: 10px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>Name:</h3></label></td>
				<td><input type="text" required name="name" placeholder="enter user name" style="margin-right: 10px;margin-top: 25px;margin-left: 10px;margin-bottom: 10px;color: #31708f;"></td></tr></div>
				<div><tr><td><label style="margin-right: 10px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>Password:</h3></label></td>
				<td><input type="password" required name="pass" placeholder="enter password" style="margin-right: 10px;margin-top: 25px;margin-left: 10px;margin-bottom: 10px;color: #31708f;"></td></tr></div>
				...
				<td><input type="email" required name="email" placeholder="enter email" style="margin-right: 10px;margin-top: 25px;margin-left: 10px;margin-bottom: 10px;color: #31708f;"></td></tr></div>
				...
				<td><input type="date" required name="dob" style="margin-right: 10px;margin-top: 25px;margin-left: 10px;margin-bottom: 10px;color: #31708f;"></td></tr></div>
				<div><tr><td><label style="margin-right: 10px;margin-top: 10px;margin-left: 10px;margin-bottom: 10px;"><h3>Country:</h3></label></td>
				<td><input type="text" required name="country" placeholder="enter country" style="margin-right: 10px;margin-top: 25px;margin-left: 10px;margin-bottom: 10px;color: #31708f;"></td></tr></div>
				</table>
