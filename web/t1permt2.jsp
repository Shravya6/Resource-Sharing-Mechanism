    ...
    <header>
       <div class="header-content">
            <div class="header-content-inner">
                <h1 id="homeHeading">RESPOND TO PERMISSION ACTIVATION REQUEST</h1>
                <hr><center>
                <table style="width: 100%" border="3">
                                ...
                                <th style="color:#ffffff;font-size: 15px">Granted</th>
                            </tr>
                            <%
                                ...
                                try {
                                    ...
                                    rs = st.executeQuery("select * from filereq where oid='" + oid + "' And status='sent' And status1='approved'");
                                    while (rs.next()) {
                            ...
                            <tr>
                                ...
                                <td><a href="t1perm.jsp?t2tenant=<%=rs.getString("t2tenant")%>&fid=<%=rs.getString("fid")%>&filename=<%=rs.getString("filename")%>&oid=<%=rs.getString("oid")%>&uid=<%=rs.getString("t2id")%>&t2mail=<%=rs.getString("t2mail")%>&fskey=<%=rs.getString("fskey")%>"><font style="color: ffffff;font-size: 15px">Granted</font></a></td>
                            </tr>
                            <%}
                                } catch (Exception ex) {
                                    ex.printStackTrace();
